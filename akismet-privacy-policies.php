...
class Akismet_Privacy_Policies
{
	...
	public function __construct()
	{
		...
		if ($this->options[ 'checkbox' ]) {
			add_action('pre_comment_on_post', array($this, 'print_error_message'));
		}
	...
	public function print_error_message() {
		...
		if (isset($_POST[ 'comment' ]) && ! isset($_POST[ 'akismet_privacy_check' ])) {
			$message = apply_filters(
				...
				esc_html__($this->options[ 'error_message' ], 'akismet-privacy-policies')
			);
			wp_die($message);
		}
	}
		...
		<div class="wrap">
			...
			<form method="post" action="options.php">
				...
				<table class="form-table">
					<tbody>
					<tr>
						<th scope="row"><label for="select_translation_language"><?php _e('Select translation language',
									'akismet-privacy-policies') ?></label></th>
						<td>
							<select id="select_translation_language" name="translation" onchange="location = location.href+'&amp;translation='+this.options[this.selectedIndex].value">
								<?php foreach ($this->languages as $lang) { ?>
									<option value="<?php echo $lang ?>" <?php selected($this->translation,
										$lang) ?>><?php echo strtoupper(substr($lang, 0, 2)) ?></option>
								<?php } ?>
							</select>
						</td>
					</tr>
					...
					<tr>
						...
						<td>
							...
							<br /><?php _e('<strong>Attention:</strong> You will have to add the link to your privacy statement manually. In Wordpress 5 and later you can find a link to a guide for creating your own privacy statement under \'Settings\' &rarr; \'Privacy\'.',
								"akismet-privacy-policies") ?>
